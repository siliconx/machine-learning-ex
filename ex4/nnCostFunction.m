...
% vectorize y
Y = zeros(m, num_labels);  % m * num_labels
for i = 1:m,
...
% compute hx
new_X = [ones(m, 1) X];  % m * (input_layer_size + 1) = m * 401

a2 = sigmoid(new_X * Theta1');  % m * hidden_layer_size = m * 25
a2 = [ones(m, 1) a2];  % m * (hidden_layer_size + 1) = m * 26
hx = a3 = sigmoid(a2 * Theta2');  % m * num_labes = m * 10
